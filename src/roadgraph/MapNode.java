...
class MapNode implements Comparable<MapNode>
{
	...
	MapNode(GeographicPoint loc)
	{
		...
		strDistance = Double.POSITIVE_INFINITY; 
	}
	...
	MapEdge getEdge(MapNode mn) {
		Set<MapEdge> me = this.edges;
		for (MapEdge m : me) {
			if (m.getStartPoint().equals(this.getLocation()) && 
					m.getEndPoint().equals(mn.getLocation())) {
				return m;
			}
		}
		return null;
	...
	public Double getStrDistance() {
		return this.strDistance;
	}
	
	public void setStrDistance(double strDistance) {
	...
	public int compareTo(MapNode mn) {
		return Double.compare(this.distance, mn.getDistance());
	}
