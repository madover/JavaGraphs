...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!toExplore.isEmpty()) {
			...
			if (!visited.contains(next)) {
				...
				Set<MapNode> neighbors = next.getNeighbors();
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						MapEdge currEdge = next.getEdge(neighbor);
						if (currEdge != null) { 
							//System.out.println(edge);
							double dist = next.getDistance() + currEdge.getLength();
							if (dist < neighbor.getDistance()) {
								neighbor.setDistance(dist);
								parentMap.put(neighbor, next);
								toExplore.add(neighbor);
		...
		/*System.out.println("\nParentMap is: ");
		for (MapNode mn : parentMap.keySet()) {
			System.out.println(parentMap.get(mn));
		}
		System.out.println("\nParentMap ended!");*/
		if (!next.equals(endNode)) {
			System.out.println("No path found from " +start+ " to " + goal);
			...
			if (!visited.contains(next)) {
				...
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						...
						if (currEdge != null) { 
							//debug code
							//System.out.println(currEdge.getCoeff() + " " + currEdge.getRoadType());
							//System.out.println(next.getDistance() + "    " + currEdge.getLength());
							double dist = next.getDistance() + currEdge.getLength();
							double strDist = dist + neighbor.getLocation().distance(endNode.getLocation());
							//System.out.println(dist + " 		" + strDist);
							if (strDist < neighbor.getStrDistance()) {
								neighbor.setDistance(dist);
								neighbor.setStrDistance(strDist);
								parentMap.put(neighbor, next);
								toExplore.add(neighbor);
							}
						}
	...
	void showGraph() {
		for (GeographicPoint gp : this.pointNodeMap.keySet()) {
			System.out.println(pointNodeMap.get(gp));
			System.out.println("\n Neighbors: ");
			for (MapNode mn : pointNodeMap.get(gp).getNeighbors()) {
				System.out.println("\n" + mn);
			}
			System.out.println("\n Neigbors done ________________________ \n");
		}
	}
	/*@Override
	public String toString() {
		...
		for (GeographicPoint gp : pointNodeMap.keySet()) {
			sb.append(pointNodeMap.get(gp));
			sb.append("\n All neighbors: ");
			Set<MapNode> n = pointNodeMap.get(gp).getNeighbors();
			for (MapNode mn : n) {
				sb.append("\n");
				sb.append(mn);
			}
